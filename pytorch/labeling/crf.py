...
def _is_transition_allowed(encoding_type, from_tag, from_label, to_tag, to_label):
    ...
    elif encoding_type == 'bieo':
        if from_tag == 'start':
            return to_tag in ['b', 'o']
        elif from_tag == 'b':
            return to_tag in ['i', 'e'] and from_label == to_label or to_tag in ['o', 'end']
        elif from_tag == 'i':
            return to_tag in ['i', 'e'] and from_label == to_label
        elif from_tag in ['e', 'o']:
            return to_tag in ['b', 'end', 'o']
        else:
            raise ValueError("Unexpect tag type {}. Expect only 'B', 'I', 'E', 'O'.".format(from_tag))
    elif encoding_type == 'bieso':
        if from_tag == 'start':
            return to_tag in ['b', 's', 'o']
        elif from_tag == 'b':
            return to_tag in ['i', 'e'] and from_label == to_label
